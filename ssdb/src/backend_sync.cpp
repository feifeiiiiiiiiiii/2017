...
void BackendSync::proc(const Link *link, const std::vector<Bytes> *req){
...
void BackendSync::_run_thread(void *arg) {
	...
	const std::vector<Bytes> req = p->req;
    delete p;
...
void BackendSync::Client::init(const std::vector<Bytes> *req) {
	last_seq = 0;
	if(req->size() > 1){
